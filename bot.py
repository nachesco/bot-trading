...
STOP_LOSS_PCT = 2.0     # Vende automáticamente si cae más del 2%
...
def analizar_y_operar(simbolo='BTC/USDT'):
    ...
    # 3. Lógica con Gestión de Riesgo (Stop Loss)
    ...
    if en_posicion:
        ...
        # CASO A1: EJECUCIÓN DE STOP LOSS (Pérdida superior al 2%)
        if porcentaje_variacion <= -STOP_LOSS_PCT:
        ...
        # CASO A2: VENTA POR ESTRATEGIA (El precio cae por debajo de la media)
        ...
        # CASO A3: MANTENER POSICIÓN (Dentro de límites aceptables)
        else:
            precio_stop = precio_compra * (1 - STOP_LOSS_PCT/100)
            mensaje = (
                f"📦 **POSICIÓN ACTIVA (PROTEGIDA CON STOP LOSS)**\n\n"
                f"• **Precio Actual:** `${precio_actual:,.2f}` | **Entrada:** `${precio_compra:,.2f}`\n"
                f"• **Nivel de Stop Loss:** `${precio_stop:,.2f}` (-{STOP_LOSS_PCT}%)\n"
        ...
        if precio_actual > media_actual:
            ...
            mensaje = (
                ...
                f"🛡️ **Stop Loss Fijo:** `${precio_stop:,.2f}` (-{STOP_LOSS_PCT}%)\n\n"
                f"💡 *Estrategia:* Cruce alcista sobre SMA 20."
            )
            enviar_telegram(mensaje)
...
if __name__ == '__main__':
    enviar_telegram(f"🛡️ **Simulador con Stop Loss ({STOP_LOSS_PCT}%) Activado**\nProtección de capital configurada.")
